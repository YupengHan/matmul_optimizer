...
int runner_main(int argc, char** argv) {
  ...
  try {
    ...
    if (args->mode == RunnerMode::kCorrectness) {
      const bool launched = launch_bf16_gemm_v1(d_a.get(), d_b.get(), d_c.get(), spec.m, spec.n, spec.k, stream.get());
      require(launched, "launch_bf16_gemm_v1 failed");
    ...
    for (int i = 0; i < args->warmup; ++i) {
      ...
      const bool launched = launch_bf16_gemm_v1(d_a.get(), d_b.get(), d_c.get(), spec.m, spec.n, spec.k, stream.get());
      require(launched, "launch_bf16_gemm_v1 failed during warmup");
    }
    ...
    for (int i = 0; i < args->iters; ++i) {
      ...
      const bool launched = launch_bf16_gemm_v1(d_a.get(), d_b.get(), d_c.get(), spec.m, spec.n, spec.k, stream.get());
      require(launched, "launch_bf16_gemm_v1 failed during timed iterations");
