...
__device__ __forceinline__ void stage_a_shared_tile_async_tile384_straight(
    __nv_bfloat16* shared_tile,
    const __nv_bfloat16* global_tile,
    int global_stride) {
  ...
  static_assert((TensorCoreTile384::kAAsyncCopiesPerTile % TensorCoreTile384::kWarpsPerBlock) == 0,
                "Tile384 A staging specialization expects a warp-symmetric copy count.");
  const int warp_id = threadIdx.x / kWarpSize;
  const int lane_id = threadIdx.x % kWarpSize;
  constexpr int kCopiesPerWarp =
      TensorCoreTile384::kAAsyncCopiesPerTile / TensorCoreTile384::kWarpsPerBlock;
  if (lane_id >= kCopiesPerWarp) {
    return;
  }

  const int copy_idx = warp_id * kCopiesPerWarp + lane_id;
  const int row = copy_idx / TensorCoreTile384::kAAsyncCopiesPerRow;
  const int col = (copy_idx % TensorCoreTile384::kAAsyncCopiesPerRow) * kAsyncCopyElems;
  cp_async_copy_16_bytes(
      shared_tile + row * kWmmaK + col,
      global_tile + row * global_stride + col);
}
...
__device__ __forceinline__ void stage_b_shared_tile_async_tile384_straight(
    __nv_bfloat16* shared_tile,
    const __nv_bfloat16* global_tile,
    int global_stride) {
  ...
  constexpr int kCopiesPerThread =
      TensorCoreTile384::kBAsyncCopiesPerTile / kTile384Threads;
  static_assert(kCopiesPerThread == 3,
                "Tile384 B staging specialization expects three async copies per thread.");
...
__global__ void bf16_gemm_v1_tensor_core_fixed_peeled_kernel(
  ...
  static_assert(TileConfig::kTensorBlockN == TensorCoreTile384::kTensorBlockN,
                "Fixed peeled hot path expects the Tile384 configuration.");

  stage_a_shared_tile_async_tile384_straight(a_shared[0], a_block, kFixedBenchmarkK);
  stage_b_shared_tile_async_tile384_straight(b_shared[0], b_block, kFixedBenchmarkN);
  cp_async_commit_group();
  stage_a_shared_tile_async_tile384_straight(a_shared[1], a_block + kWmmaK, kFixedBenchmarkK);
  stage_b_shared_tile_async_tile384_straight(
      b_shared[1],
  ...
  for (int tile_idx = 0; tile_idx < FixedKTiles - 2; tile_idx += 2) {
    ...
    __syncthreads();
    stage_a_shared_tile_async_tile384_straight(
