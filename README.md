...
The tree asset below is regenerated from the latest tracked round history in the repo, which now spans `286` recorded measurement rounds, so it reflects the latest exploratory commits while keeping the official best snapshot anchored to the current recorded-best commit `489574e`.
