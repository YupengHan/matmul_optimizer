...
static_assert(sizeof(CutlassElement) == sizeof(std::uint16_t), "CUTLASS BF16 must be 16-bit");
...
void check_cutlass(cutlass::Status status, const std::string& context) {
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error(context + ": " + cutlass::cutlassGetStatusString(status));
  }
}
...
std::vector<T> read_binary_file(const fs::path& path, std::size_t expected_count) {
  ...
  require(fs::file_size(path) == expected_bytes,
          "unexpected binary size for " + path.string());
...
void validate_cutlass_kernel_resources() {
  int device = 0;
  check_cuda(cudaGetDevice(&device), "cudaGetDevice");

  int max_optin_smem_bytes = 0;
  check_cuda(
      cudaDeviceGetAttribute(
          &max_optin_smem_bytes,
          cudaDevAttrMaxSharedMemoryPerBlockOptin,
          device),
      "cudaDeviceGetAttribute(max shared memory opt-in)");

  constexpr int kSharedStorageBytes = static_cast<int>(sizeof(typename CutlassGemm::GemmKernel::SharedStorage));
  if (kSharedStorageBytes > max_optin_smem_bytes) {
    std::ostringstream message;
    message << "CUTLASS kernel requires " << kSharedStorageBytes
            << " bytes of dynamic shared memory, but device opt-in limit is "
            << max_optin_smem_bytes;
    throw std::runtime_error(message.str());
  }
}

void launch_cutlass_bf16_gemm(
    ...
    int k,
    cudaStream_t stream) {
  const auto* a = reinterpret_cast<const CutlassElement*>(a_bf16);
  const auto* b = reinterpret_cast<const CutlassElement*>(b_bf16);
  auto* c = reinterpret_cast<CutlassElement*>(c_bf16);
  ...
  validate_cutlass_kernel_resources();
  check_cutlass(CutlassGemm::can_implement(args), "CUTLASS GEMM rejected problem");
  check_cutlass(gemm_operator(args, nullptr, stream), "CUTLASS GEMM launch failed");
}
...
int runner_main(int argc, char** argv) {
  ...
  try {
    ...
    if (args->mode == RunnerMode::kCorrectness) {
      launch_cutlass_bf16_gemm(d_a.get(), d_b.get(), d_c.get(), spec.m, spec.n, spec.k, stream.get());
